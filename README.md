...
`data/members_enriched.csv` — 162 *Human Factors* reviewers with role code, expertise
...
87 carry the code **`Not_EB`**: prolific authors (>3 papers in the
...
- `python aggregate_not_eb.py` rebuilds every `Not_EB` row from the per-author profiles
  ...
  profiles are added or corrected. It also **de-duplicates against the editorial
  board**: any `Not_EB` author who is the same person as an AE/EB/PR/R reviewer
  (nickname-aware, e.g. "McDonald, Anthony D." == PR "McDonald, Tony") is dropped,
  keeping the editorial-board entry. Run `python precompute.py` afterward to refresh
